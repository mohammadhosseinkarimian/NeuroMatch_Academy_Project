...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset(
        ...
        *,
        ...
        self.meteors = []
        self.meteors.append((360,175))
        self.meteors.append((210,75))

        self.sky_polys = []
        for i in range(CHUNKS - 1):
            p1 = (chunk_x[i] , smooth_y[i])
            p2 = (chunk_x[i + 1] , smooth_y[i + 1] )
            self.moon.CreateEdgeFixture(vertices=[p1, p2], density=0, friction=0.4)
    ...
    def step(self, action):
        ...
        rngx =  pos.x * SCALE - 30
        rngy = 370 - pos.y * SCALE
        #print("X IS : " + str(rngx) + " Y is : " + str(rngy))
        for i in self.meteors:
            mx = i[0] - 5
            my = i[1] - 3
            if rngy > (my-30) and rngy < (my+60) and rngx < (mx+70) and rngx > (mx-50):
                #print("************************")
                #print(rngx, rngy)
                #print("************************")
                reward = -2
    ...
    def _render(self, mode="human"):
        ...
        for i in range(2):
            ...
            p = self.meteors[i]
            image = pygame.image.load('m1.png').convert_alpha()
            
            image = pygame.transform.rotozoom(image, 0, 0.5)
            self.screen.blit(image, (p[0],p[1])) # paint to screen
            if(p[0]<-57) :
                t = ( 620 , p[1])
                self.meteors[i] = t
            if(p[0]>620) :
        ...
        #image2 = pygame.image.load('m1.png').convert_alpha()
        #image2 = pygame.transform.rotozoom(image2, 0, 0.5)
        #self.screen.blit(image2, (390, 70))  # paint to screen
        pygame.display.flip()
